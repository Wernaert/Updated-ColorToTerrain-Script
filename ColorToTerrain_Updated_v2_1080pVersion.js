...
// script.param.defaultMats.description=Bare Grass, Coarse Dirt, Grass Path, Podzol, Stone, Granite, Deepslate, Blackstone, Snow, Water, Lava, Netherrack, Soul Sand, Warped Nylium and Crimson Nylium 
...
// script.param.cobblestone.displayName=Cobblestone + Mossy Cobblestone

// script.param.soulSoilMyceliumEndStone.type=boolean
// script.param.soulSoilMyceliumEndStone.default=false
// script.param.soulSoilMyceliumEndStone.displayName=Soul Soil + Mycelium + End Stone

// script.param.obsidianBedrock.type=boolean
// script.param.obsidianBedrock.default=false
// script.param.obsidianBedrock.displayName=Obsidian + Bedrock
...
var useObsidianBedrock = params['obsidianBedrock'];
var useGravel = params['gravel'];
var useClay = params['clay'];
var useSoulSoilMyceliumEndStone = params['soulSoilMyceliumEndStone'];
...
if (arguments[0] == "list") {
	...
	if (arguments.length < 2 || arguments[1] == "" || arguments[1] == "default") {
		...
		var defaultMatsArray = ("BareGrass 69 110 51\n Permadirt 134 96 67\nPodzol 90 63 28\nStone 125 125 125\nWater 47 67 244\nLava 216 104 26\n\nNetherrack 111 54 52\nWarped_Nylium 43 114 101\nCrimson_Nylium 131 31 31\nGranite 153 113 98\nDeepSnow 239 251 251\nGrassPath 159 114 98\nDeepslate 80 80 83\nBlackstone 42 36 41\nSoulSand 84 64 51").split("\n");

		var cobblestoneArray = ("Cobblestone 122 122 122\nMossyCobblestone 103 121 103").split("\n");

		var obsidianBedrockArray = ("Obsidian 20 18 29\nBedrock 83 83 83").split("\n");

		var soulSoilMyceliumEndStoneArray = ("Soul_Soil 76 58 47\nMycelium 111 99 105\nEndStone 221 223 165").split("\n");
		...
		if (useCobblestone) {
			def = def.concat(cobblestoneArray);
		}

		if (useObsidianBedrock) {
			def = def.concat(obsidianBedrockArray);
		}

		if (useSoulSoilMyceliumEndStone) {
			def = def.concat(soulSoilMyceliumEndStoneArray);
		}
